document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchStats() {
        try {
            ...
            // 1. Update Metrics (Top)
            document.getElementById('r2-score').textContent = data.r2;
            document.getElementById('rmse-score').textContent = data.rmse;
            document.getElementById('pearson-score').textContent = data.pearson;

            // 2. Update Metrics (Sidebar)
            document.getElementById('r2-score-sidebar').textContent = data.r2;
            document.getElementById('rmse-score-sidebar').textContent = data.rmse;
            document.getElementById('pearson-score-sidebar').textContent = data.pearson;

            // 3. Populate Input Form
            const featuresContainer = document.getElementById('featuresContainer');
            featuresContainer.innerHTML = ''; // Clear loading state

            Object.keys(data.features).forEach(feat => {
                const div = document.createElement('div');
                div.className = 'col-span-1 group';
                div.innerHTML = `
                    <label for="${feat}" class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2 group-focus-within:text-accent-600 transition-colors">
                        ${feat.replace(/_/g, ' ')}
                    </label>
                    <input type="number" step="any" id="${feat}" name="${feat}" required min="0" max="100"
                        class="w-full bg-slate-50 border border-slate-200 text-brand-900 font-medium rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-accent-500/20 focus:border-accent-500 transition-all placeholder:text-slate-400"
                        placeholder="0">
                `;
                featuresContainer.appendChild(div);
            });

            // 4. Populate Feature Importance
            const featureList = document.getElementById('featureImportanceList');
            featureList.innerHTML = ''; // Clear loading state

            Object.entries(data.features).forEach(([feat, val]) => {
                const percentage = (val * 100).toFixed(1) + '%';
                const div = document.createElement('div');
                div.className = 'relative';
                div.innerHTML = `
                    <div class="flex justify-between text-sm font-medium mb-1">
                        <span class="text-slate-700">${feat.replace(/_/g, ' ')}</span>
                        <span class="text-slate-900">${percentage}</span>
                    </div>
                    <div class="w-full bg-slate-100 rounded-full h-2.5">
                        <div class="bg-gradient-to-r from-accent-500 to-tech-500 h-2.5 rounded-full" style="width: ${percentage}"></div>
                    </div>
                `;
